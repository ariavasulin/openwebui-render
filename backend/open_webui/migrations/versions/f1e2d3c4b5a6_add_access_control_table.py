"""Add access_control table

Revision ID: f1e2d3c4b5a6
Revises: 8452d01d26d7
Create Date: 2026-02-05 10:00:00.000000

Migrates from JSON access_control columns to normalized access_control table.
...
def upgrade() -> None:
    ...
    if "access_control" not in existing_tables:
        op.create_table(
            "access_control",
    ...
    for table_name, resource_type in resource_tables:
        ...
        for row in rows:
                ...
                if key not in inserted:
                    try:
                        conn.execute(
                            sa.text(
                                """
                                INSERT INTO access_control (id, resource_type, resource_id, principal_type, principal_id, permission, created_at)
            ...
            # Extract permissions and insert into access_control table
...
def downgrade() -> None:
    ...
    # Step 2: Query access_control table and reconstruct JSON for each resource
    for table_name, resource_type in resource_tables:
        try:
            ...
            try:
                conn.execute(
                    sa.text(f'''
                        ...
                        WHERE id NOT IN (
                            SELECT DISTINCT resource_id FROM access_control WHERE resource_type = :resource_type
                        )
    ...
    # Step 5: Drop the access_control table
    op.drop_index("idx_access_control_principal", table_name="access_control")
    op.drop_index("idx_access_control_resource", table_name="access_control")
